...
export default function SubmitPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [isParticipationRequestFormOpen, setIsParticipationRequestFormOpen] = useState(true);
  const {
  ...
  useEffect(() => {
    ...
    setIsParticipationRequestFormOpen(participations.length === 0);
  }, [isEditionLoading, isLoading, participations.length]);
          ...
          <div className='flex items-start justify-between gap-3'>
            ...
            {participations.length > 0 ? (
              <Button
                ...
                variant='outline'
                size='sm'
                onClick={() => setIsParticipationRequestFormOpen((current) => !current)}
                aria-expanded={isParticipationRequestFormOpen}
                aria-controls='participation-request-form'
              >
                {isParticipationRequestFormOpen ? (
                  <>
