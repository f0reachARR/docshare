...
export const competitionSeries = pgTable("competition_series", {
  ...
  externalLinks:
    jsonb("external_links").$type<Array<{ label: string; url: string }>>(),
  createdAt: timestamp("created_at", { withTimezone: true })
    .notNull()
    .defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true })
...
export const competitionEditions = pgTable(
  "competition_edition",
  {
    ...
    description: text("description"),
    ruleDocuments:
      jsonb("rule_documents").$type<
        Array<{ label: string; s3_key: string; mime_type: string }>
      >(),
  ...
  (table) => ({
    seriesYearIdx: index("competition_edition_series_year_idx").on(
      table.seriesId,
      table.year,
    ),
  }),
...
export const submissions = pgTable(
  ...
  (table) => ({
    templateParticipationUnique: uniqueIndex(
      "submission_template_participation_unique",
    ).on(table.templateId, table.participationId),
    participationIdx: index("submission_participation_idx").on(
...
export const comments = pgTable(
  ...
  (table) => ({
    ...
    editionDeletedIdx: index("comment_edition_deleted_idx").on(
      table.editionId,
      table.deletedAt,
    ),
  }),
