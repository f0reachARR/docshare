...
### 5.1 他校資料の閲覧条件（資料種別S単位）

大会回 E・資料種別 S において、ユーザー U が他校の資料を閲覧できる条件:

```
1. E の sharing_status が "sharing" または "closed" である
2. U が現在選択中 organization X の member である
3. X が E に participation を持つ
4. X の participation に template S の submission が1つ以上ある
```

条件を満たした場合のみ、資料種別 S の他校資料を閲覧可能。`submitted = true` でも条件未達なら `viewable = false` とし、理由コードを返す。
...
| 操作 | 条件 |
|------|------|
| コメント閲覧 | 指定した資料種別 S の資料閲覧権限と同一 |
| コメント投稿 | 指定した資料種別 S の資料閲覧権限を持つログインユーザー |
| コメント編集 | コメント投稿者本人のみ |
...
```typescript
async function canViewOtherSubmissionsByTemplate(
  userId: string,
  editionId: string,
  templateId: string,
  ...
  const participation = await getParticipation(organizationId, editionId);
  if (!participation) return false;

  return hasTemplateSubmission(participation.id, templateId);
}

// コメント投稿可否 = 閲覧権限と同一
const canComment = canViewOtherSubmissionsByTemplate;
```
