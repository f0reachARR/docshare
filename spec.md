...
大会回 E において、ユーザー U が他校の資料種別 S を閲覧できる条件:

```
1. E の sharing_status が "sharing" または "closed" である
2. 現在選択されている大学 X が大会回 E に出場登録している (participation テーブル)
3. 現在選択されている大学 X の出場登録に紐づく資料種別 S の submission が1つ以上存在する
4. U が現在選択されている大学 X に所属している (member テーブル)
```

条件を満たした場合、大会回 E の資料種別 S の他校資料を閲覧可能。

他校へのコメントは資料本体とは別判定とし、現在選択されている大学 X が大会回 E において1件以上の submission を持ち、かつ U が大学 X に所属している場合に閲覧・投稿可能。
